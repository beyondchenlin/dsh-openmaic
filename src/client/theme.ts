/**
 * Host-theme resolution for the teaching card: reads the DSH `--dsw-alias-*`
 * design tokens (whale-blue brand accent included) off `document.body` and
 * derives the host color scheme. The body is the read point because DSH mounts
 * the token definitions there; missing tokens resolve to empty strings the
 * shell drops, so the frame stylesheet's `light-dark()` fallbacks apply
 * outside DSH.
 *
 * @module @openmaic/dsh-openmaic/theme
 */
